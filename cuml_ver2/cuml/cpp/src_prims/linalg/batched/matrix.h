 ...
 *       column-wise.
 * 
 * @param[in]  in1            Input matrix1
 * @param[in]  in2            Input matrix2
 * @param[out] out           Output matrix
 * @param[in]  in_rows       Number of rows in an input matrix
 * @param[in]  in_cols       Number of columns in an input matrix
 * @param[in]  out_rows      Number of rows to copy
 * @param[in]  out_cols      Number of columns to copy
 * @param[in]  bias_len
 */
template <typename T>
static __global__ void concatenate_kernel(const T* in1, const T* in2, T* out,
                                             int in_rows, int in_cols,
                                             int out_rows, int out_cols, int bias_len) {
  const T* in1_ = in1 + blockIdx.x * in_rows * in_cols;
  const T* in2_ = in2 + blockIdx.x * in_rows * in_cols;

  T* out_ = out + blockIdx.x * out_rows * out_cols;

  int num_elems = out_rows*(out_cols+bias_len);
  int in1_size = in_rows*in_cols;

  for(int i = threadIdx.x; i<num_elems; i+=blockDim.x){
    int i_col = i / out_rows;
    int i_row = i % out_rows;
    if(i<in1_size){
      out_[i] = in1_[i_row + in_rows * i_col];
    }else if(i<2*in1_size){
      out_[i] = in2_[i_row + in_rows * i_col - (num_elems/2)];
    }else{
      out_[i] = (T)1;
    }
  }
...
template <typename T>
Matrix<T> concatenate(const Matrix<T>& in1, const Matrix<T>& in2, int rows, int cols, int bias_len) {

  Matrix<T> out(rows, cols+bias_len, in1.batches(), in1.cublasHandle(), in1.allocator(),
                in1.stream(), false);
  // Execute the kernel
  const int TPB = rows * (cols+bias_len) > 512 ? 256 : 128;  // quick heuristics
  concatenate_kernel<<<in1.batches(), TPB, 0, in1.stream()>>>(
    in1.raw_data(), in2.raw_data(), out.raw_data(), 
    in1.shape().first, in1.shape().second, rows, cols, bias_len);
