...
void _granger_least_squares(cumlHandle& handle, 
  ...
  std::cout << "_granger_least_squares settings finished" << std::endl;   
  ...
  std::cout << "lagged X_mt finished" << std::endl;

  //use only the last (p-1) columns for both X1 and X2 as A
  // X1_fit = X1[:,1:]; X2_fit = X2[:,1:];
  MLCommon::LinAlg::Batched::Matrix<double> X1_fit =
    MLCommon::LinAlg::Batched::b_2dcopy(X1_mt, 0, 1, rows, p);

  MLCommon::LinAlg::Batched::Matrix<double> X2_fit =
    MLCommon::LinAlg::Batched::b_2dcopy(X2_mt, 0, 1, rows, p);
  std::cout << "cut X1&2_fit finished" << std::endl;
  ...
  MLCommon::LinAlg::Batched::Matrix<double> X_fit =
    MLCommon::LinAlg::Batched::concatenate(X1_fit, X2_fit, rows, cols, bias_len);
  std::cout << "concatenated X_fit finished" << std::endl;

  // Generate true outputs Y for the model fit
  MLCommon::LinAlg::Batched::Matrix<double> Y =
    MLCommon::LinAlg::Batched::b_2dcopy(X1_mt, 0, 0, rows, 1);
  std::cout << "cut Y finished" << std::endl;

  // initialize a residual
  MLCommon::LinAlg::Batched::Matrix<double> Y_residual(
    rows, 1, batch_size, cublas_handle, allocator, stream, false);
  std::cout << "Y_residual created" << std::endl;

  if (estimate) {
    MLCommon::copy(Y_residual.raw_data(), Y.raw_data(),
                   rows * batch_size, stream);
  }
  std::cout << "Y_residual updated" << std::endl;
  ...
  std::cout << "b_gels finished" << std::endl;

  // Cut results into the weight vectors
  MLCommon::LinAlg::Batched::Matrix<double> weight =
    MLCommon::LinAlg::Batched::b_2dcopy(Y, 0, 0, rows, 1);
  std::cout << "weight created" << std::endl;

  if (estimate) {
    // Compute final residual (technically a gemv)
    MLCommon::LinAlg::Batched::b_gemm(false, false, cols, 1 , cols,
...
void granger_causality_test(cumlHandle& handle, GrangerOrder order,
                 const double* d_x1, const double* d_x2, bool estimate, int batch_size, int n) {
  std::cout << "entered granger_causality_test" << std::endl;
  ML::PUSH_RANGE(__func__);
  const auto& handle_impl = handle.getImpl();
  auto stream = handle_impl.getStream();
  auto cublas_handle = handle_impl.getCublasHandle();
  ...
  std::cout << "granger_causality_test settings finished" << std::endl;
  // prepare X1 and X2
  MLCommon::LinAlg::Batched::Matrix<double> X1(
    1, n, 1, cublas_handle,allocator, stream, true);
  MLCommon::LinAlg::Batched::Matrix<double> X2(
    1, n, 1, cublas_handle,allocator, stream, true  );
  std::cout << "created X1 & X2" << std::endl;

  MLCommon::TimeSeries::prepare_data(X1.raw_data(), d_x1, batch_size, n, 0, 0, 0, stream);
  std::cout << "prepared X1" << std::endl;
  MLCommon::TimeSeries::prepare_data(X2.raw_data(), d_x2, batch_size, n, 0, 0, 0, stream);
  std::cout << "prepared X2" << std::endl;


  // Do the computation of the initial parameters
  _granger_least_squares(handle, X1, X2, order.p, order.q, estimate, order.bias_len);
