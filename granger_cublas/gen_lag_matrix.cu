...
__global__ void lag_matrix_kernel(const double* X1, const double* X2, double* lag_matrix, double* y_label, double bias, int rows, int cols, int p){
...
// 
void gen_lag_matrix(const double* x1, const double* x2, double* lag_matrix, double* y_label, double bias,
                int n, int p, int rows, int cols, int threads_per_block){
    ...
    lag_matrix_kernel<<<num_blocks,threads_per_block>>>(d_x1, d_x2, d_matrix_lag, d_label, bias, rows, cols, p);
