...
void gen_lag_matrix(const double* x1, const double* x2, double* lag_matrix, double* lag_x1, double* y_label, 
    ...
    double * d_x1_lag = new double[rows*p];
    ...
    cudaMallocManaged((void**)&d_x1_lag, sizeof(double)*(rows*p));
    ...
    cudaMemcpy(lag_x1, d_x1_lag, (rows*p)*sizeof(double), cudaMemcpyDeviceToHost);
